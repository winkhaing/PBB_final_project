...
# Load the data
health_spending = pd.read_csv("data\\total_health_spending_percent_of_gdp.csv")
infant_mortality = pd.read_csv("data\\sp_dyn_imrt_in.csv")
...
health_asean = health_spending[health_spending["country"].isin(asean_countries)]
mortality_asean = infant_mortality[infant_mortality["country"].isin(asean_countries)]

# Get ISO3 codes for mapping
iso3_map = coco.convert(names=asean_countries, to="ISO3")
country_iso = dict(zip(asean_countries, iso3_map))

# Prepare data for 1995 and 2010 comparison
...
health_time = health_asean[["country"] + years].melt(
...
mortality_time = mortality_asean[["country"] + years].melt(
    id_vars=["country"],
    value_vars=years,
    var_name="year",
...
# Create choropleth maps focused on ASEAN region
# Define ASEAN region boundaries for map focus
asean_bounds = {"lat_min": -10, "lat_max": 28, "lon_min": 92, "lon_max": 141}

health_map_1995 = px.choropleth(
    merged_1995_2010,
    locations="iso_alpha",
    color="1995_health",
    hover_name="country",
    # title="Health Spending (% of GDP) in 1995",
    color_continuous_scale="Blues",
    # labels={"1995_health": "Health Spending (% of GDP)"},
    scope="asia",
)
health_map_1995.update_layout(
...
health_map_2010 = px.choropleth(
    merged_1995_2010,
    locations="iso_alpha",
    color="2010_health",
    hover_name="country",
    # title="Health Spending (% of GDP) in 2010",
    color_continuous_scale="Blues",
    # labels={"2010_health": "Health Spending (% of GDP)"},
    scope="asia",
)
health_map_2010.update_layout(
    coloraxis_showscale=True,
    geo=dict(
        projection_scale=1,
        center=dict(lat=8, lon=115),
...
mortality_map_1995 = px.choropleth(
    merged_1995_2010,
    locations="iso_alpha",
    color="1995_mortality",
    hover_name="country",
    # title="Infant Mortality Rate in 1995",
    color_continuous_scale="Reds",
    # labels={"1995_mortality": "Infant Mortality Rate"},
    scope="asia",
)
mortality_map_1995.update_layout(
    coloraxis_showscale=True,
    geo=dict(
        projection_scale=1,
        center=dict(lat=8, lon=115),
...
mortality_map_2010 = px.choropleth(
    merged_1995_2010,
    locations="iso_alpha",
    color="2010_mortality",
    hover_name="country",
    # title="Infant Mortality Rate in 2010",
    color_continuous_scale="Reds",
    # labels={"2010_mortality": "Infant Mortality Rate"},
    scope="asia",
)
mortality_map_2010.update_layout(
    coloraxis_showscale=True,
    geo=dict(
        projection_scale=1,
        center=dict(lat=8, lon=115),
...
# Create bar charts for comparison with improved layout
health_bar = px.bar(
    merged_1995_2010,
    x="country",
    y=["1995_health", "2010_health"],
    barmode="group",
    title="Health Spending (% of GDP): 1995 vs 2010",
    labels={"value": "Health Spending (% of GDP)", "variable": "Year"},
...
health_bar.update_traces(textposition="outside")

mortality_bar = px.bar(
    merged_1995_2010,
    x="country",
    y=["1995_mortality", "2010_mortality"],
    barmode="group",
    title="Infant Mortality Rate: 1995 vs 2010",
    labels={"value": "Infant Mortality Rate", "variable": "Year"},
    color_discrete_sequence=["#cb181d", "#fb6a4a"],
    text_auto=".1f",
)
mortality_bar.update_layout(
    legend_title_text="Year",
    xaxis_title="Country",
...
mortality_bar.update_traces(textposition="outside")

# Create time series plots with improved styling
...
mortality_time_series = px.line(
    ...
    y="infant_mortality",
    color="country",
    title="Infant Mortality Rate Over Time (1995-2010)",
...
# Create animated choropleth maps focused on ASEAN region
animated_health_map = px.choropleth(
    time_series,
    locations="iso_alpha",
    color="health_spending",
    animation_frame="year",
    hover_name="country",
    # title="Health Spending (% of GDP) Evolution (1995-2010)",
    color_continuous_scale="Blues",
    range_color=[1.5, 7],
    # labels={"health_spending": "Health Spending (% of GDP)"},
    scope="asia",
)
animated_health_map.update_layout(
    coloraxis_showscale=True,
    # Fix overlapping legend title
    coloraxis=dict(
        colorbar=dict(
            title=dict(
                text="Health Spending (% of GDP)",  # Add line break in title
                font=dict(size=12),  # Smaller font
                side="right",  # Position to the right
            ),
            thickness=15,  # Slightly thicker bar
            len=0.7,  # Shorter color bar
            x=1.02,  # Move slightly to the right
            y=0.5,  # Center vertically
            tickfont=dict(size=9),  # Smaller tick font
        )
    ),
    geo=dict(
        projection_scale=1,
...
# Adjust play/pause button duration for clear visibility
animated_health_map.layout.updatemenus[0].buttons[0].args[1]["frame"]["duration"] = 800
animated_health_map.layout.updatemenus[0].buttons[0].args[1]["transition"][
    "duration"
] = 300

animated_mortality_map = px.choropleth(
    time_series,
    locations="iso_alpha",
    color="infant_mortality",
    animation_frame="year",
    hover_name="country",
    # title="Infant Mortality Rate Evolution (1995-2010)",
    color_continuous_scale="Reds",
    range_color=[0, 90],
    # labels={"infant_mortality": "Infant Mortality Rate"},
    scope="asia",
)
animated_mortality_map.update_layout(
    coloraxis_showscale=True,
    # Fix overlapping legend title
    coloraxis=dict(
        colorbar=dict(
            title=dict(
                text="Infant Mortality Rate",  # Add line break in title
                font=dict(size=12),  # Smaller font
                side="right",  # Position to the right
            ),
            thickness=15,  # Slightly thicker bar
...
# Add play/pause button styling
animated_mortality_map.layout.updatemenus[0].buttons[0].args[1]["frame"][
    "duration"
] = 800
animated_mortality_map.layout.updatemenus[0].buttons[0].args[1]["transition"][
    "duration"
] = 300

# Create scatter plot for relationship analysis with improved styling and trendline
...
# Update trendline color and style if present
...
# Create change analysis charts with improved layout
health_change_bar = px.bar(
    merged_1995_2010.sort_values("health_change_pct"),
    x="country",
    y="health_change_pct",
    title="Percentage Change in Health Spending (1995-2010)",
...
mortality_change_bar = px.bar(
    ...
    title="Percentage Change in Infant Mortality (1995-2010)",
...
# Create animated bubble chart showing relationship over time with improved styling
animated_scatter = px.scatter(
    time_series,
    x="health_spending",
    y="infant_mortality",
    animation_frame="year",
    size=[12] * len(time_series),  # Larger fixed size for better visibility
    color="country",
    hover_name="country",
    text="country",  # Add country labels
    # title="Health Spending vs Infant Mortality (1995-2010)",
    labels={
...
# Show visible x-axis and y-axis lines
animated_scatter.update_xaxes(
    showline=True, linewidth=1, linecolor="black", mirror=True
)
animated_scatter.update_yaxes(
    showline=True, linewidth=1, linecolor="black", mirror=True
)
...
comparison_table = pd.DataFrame(
    {
        "Country": merged_1995_2010["country"],
        "Health Spending 1995 (%)": merged_1995_2010["1995_health"].round(2),
        "Health Spending 2010 (%)": merged_1995_2010["2010_health"].round(2),
        "Health Change (%)": merged_1995_2010["health_change_pct"].round(1),
        "IMR 1995": merged_1995_2010["1995_mortality"].round(1),
        "IMR 2010": merged_1995_2010["2010_mortality"].round(1),
        "IMR Change (%)": merged_1995_2010["mortality_change_pct"].round(1),
    }
)

# Create additional efficiency metric
...
# Sort the table by efficiency improvement
...
::: {.valuebox icon="heart" color="#4682B4" title="Health Spending in ASEAN (1995)"}
<span style="font-size: 1.5rem;">`{python} f"{avg_health_1995:.2f}%"`
Average % of GDP
:::

::: {.valuebox icon="heart" color="#6CA6CD" title="Health Spending in ASEAN (2010)"}
<span style="font-size: 1.5rem;">`{python} f"{avg_health_2010:.2f}%"`
Average % of GDP
:::

::: {.valuebox icon="activity" color="#B22222" title="Infant Mortality in ASEAN (1995)"}
<span style="font-size: 1.5rem;">`{python} f"{avg_mortality_1995:.1f}"`
Per 1,000 live births
:::

::: {.valuebox icon="activity" color="#CD5C5C" title="Infant Mortality in ASEAN (2010)"}
<span style="font-size: 1.5rem;">`{python} f"{avg_mortality_2010:.1f}"`
Per 1,000 live births
:::
...
::: {.valuebox icon="graph-up" color="#4169E1" title="Highest Health Spending (1995)"}
`{python} f"{max_health_1995['1995_health']:.2f}%"`
`{python} max_health_1995['country']`
:::

::: {.valuebox icon="graph-up" color="#4169E1" title="Highest Health Spending (2010)"}
`{python} f"{max_health_2010['2010_health']:.2f}%"`
`{python} max_health_2010['country']`
:::

::: {.valuebox icon="graph-down" color="#4682B4" title="Lowest Health Spending (1995)"}
`{python} f"{min_health_1995['1995_health']:.2f}%"`
`{python} min_health_1995['country']`
:::

::: {.valuebox icon="graph-down" color="#4682B4" title="Lowest Health Spending (2010)"}
`{python} f"{min_health_2010['2010_health']:.2f}%"`
`{python} min_health_2010['country']`
:::

## Row 2 {height=35%}

### Health Spending Maps {.tabset}
...
### Health Spending Bar Chart Comparison
```{python}
health_bar
```
...
### Health Spending % Change (1995-2010)
```{python}
health_change_bar
```
...
::: {.valuebox icon="heart-pulse" color="#B22222" title="Highest Mortality (1995)"}
`{python} f"{max_mortality_1995['1995_mortality']:.1f}"`
`{python} max_mortality_1995['country']`
:::

::: {.valuebox icon="heart-pulse" color="#B22222" title="Highest Mortality (2010)"}
`{python} f"{max_mortality_2010['2010_mortality']:.1f}"`
`{python} max_mortality_2010['country']`
:::

::: {.valuebox icon="heart" color="#CD5C5C" title="Lowest Mortality (1995)"}
`{python} f"{min_mortality_1995['1995_mortality']:.1f}"`
`{python} min_mortality_1995['country']`
:::

::: {.valuebox icon="heart" color="#CD5C5C" title="Lowest Mortality (2010)"}
`{python} f"{min_mortality_2010['2010_mortality']:.1f}"`
`{python} min_mortality_2010['country']`
:::

## Row 2 {height=35%}

### Infant Mortality Maps {.tabset}
...
### Infant Mortality Bar Chart Comparison
```{python}
mortality_bar
```
...
### Infant Mortality % Change (1995-2010)
```{python}
...
### Relationship Between Health Spending and Infant Mortality {.tabset}
...
### ASEAN Healthcare Efficiency Analysis
```{python}
# Create bar chart to visualize healthcare efficiency improvement
efficiency_fig = px.bar(
    comparison_table_sorted,
    x='Country',
    y='Efficiency Improvement (%)',
    title='Healthcare Efficiency Improvement (1995-2010)',
    text_auto='.1f',
